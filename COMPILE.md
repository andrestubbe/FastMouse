# Building FastMouse from Source
...
- Uses `native\fastmouse.def` for JNI exports
- Outputs to `build\fastmouse.dll`

The Maven `pom.xml` will automatically pick up `build\fastmouse.dll` and bundle it inside the JAR.

## JNI Exports (.def File)

When using JNI, you MUST export your native functions in the `native\fastmouse.def` file:

```def
LIBRARY fastmouse
EXPORTS
    Java_fastmouse_FastMouseImpl_nativeInitialize
    Java_fastmouse_FastMouseImpl_nativeStartListening
    Java_fastmouse_FastMouseImpl_nativeStopListening
    Java_fastmouse_FastMouseImpl_nativeGetConnectedDevices
    Java_fastmouse_FastMouseImpl_nativeGetDeviceName
    Java_fastmouse_FastMouseImpl_nativeGetDeviceButtonCount
```
